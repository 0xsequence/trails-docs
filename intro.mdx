...
Trails is free to integrate and is optimized for a variety of use cases for the most common flows, for example:
...
- **[Checkout](/use-cases/checkout)**: Accept crypto at checkout and settle in USD on your own Stripe account through [Stripe checkout](/sdk/stripe/checkout). One widget prop, no merchant backend, Stripe-hosted receipts.
- **[Swap](/use-cases/swap)**: Embed low-latency, highly liquid cross-chain token swaps for your applications.
- **[Fund](/use-cases/fund)**: Maximize TVL and transaction velocity by funding via fiat (card onramps through [Stripe](/sdk/stripe/onramp) and Meld, exchange transfers through Mesh) or crypto such as perpetual exchanges, prediction markets, and chains. Utilizing our intent address primitive, we are able to support nearly any token or chain - even if the onramp provider doesn't.
