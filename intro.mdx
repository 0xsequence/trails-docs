...
Trails is an intent-powered orchestration protocol that enables 1-click transactions from any wallet seamlessly with unified user liquidity across all chains.
...
**Wallet Support**: Trails works with any wallet including EOA wallets (MetaMask), smart wallets (Coinbase Smart Wallet), and embedded wallets (Privy). EIP-7702 is **not required**.

**Chain Support**: Trails supports any EVM-compatible chain. See [supported chains](/resources/supported-chains) for the full list.
...
The Trails documentation is organized to help you find what you need:

| Section | Purpose |
|---------|---------|
| **[Use Cases](/use-cases/pay)** | Business context, when to use each mode, and example scenarios |
| **[SDK Modes](/sdk/modes/pay)** | Technical reference with configuration options and code examples |
| **[SDK Hooks](/sdk/hooks)** | Headless implementation using React hooks for custom UIs |
| **[API Reference](/api-reference/introduction)** | Direct API integration for server-side or non-React implementations |
| **[Architecture](/architecture/how-it-works)** | Deep dive into how Trails works under the hood |
