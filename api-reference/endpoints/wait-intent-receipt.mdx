...
<Note>
`done` tracks the intent's on-chain status only. For [settlement intents](/api-reference/endpoints/get-intent-receipt#settlement) (Stripe checkout),
the processor confirmation arrives after `done: true`, and settlement-only changes do not wake this long-poll. Poll
