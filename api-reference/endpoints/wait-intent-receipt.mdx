...
<Note>
`done` tracks the intent's on-chain status only. For [Stripe settlement intents](/api-reference/endpoints/get-intent-receipt#stripe-settlement) (Stripe checkout),
Stripe's confirmation arrives after `done: true`, and settlement-only changes do not wake this long-poll. Poll
[`GetIntentReceipt`](/api-reference/endpoints/get-intent-receipt) for `settlement.terminal` once the intent completes.
</Note>
