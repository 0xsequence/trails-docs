...
- **settlement** (IntentSettlement): External processor settlement, present when the intent was quoted with `settlement`. See [Settlement](#settlement)
...
Intents created from a [settlement quote](/api-reference/endpoints/quote-intent#settlement-quotes) (the widget's
[Stripe checkout](/sdk/stripe/checkout)) carry a `settlement` object computed at read time. The intent reaches `SUCCEEDED` when the destination
...
|-------|------|-------------|
| `provider` | `"stripe"` | Processor confirming the deposit |
| `label`, `description` | `string` | Step copy for transaction-step UIs |
...
| `chainId` | `number` | Chain the deposit landed on |
| `status` | `string` | Raw Stripe payment status, as last observed |
| `settled` | `boolean` | Stripe confirmed the deposit |
| `failed` | `boolean` | The payment reached a terminal non-settled state (e.g. the PaymentIntent was canceled) and will never settle |
| `terminal` | `boolean` | The settlement will not change again: settled, failed, or retired with an unknown outcome (`terminal` true, `settled` and `failed` both false). Key terminality on this, never on `status`, and gate fulfillment on `settled` |
| `externalUrl` | `string` | Stripe-hosted receipt page, once available |

Verify a checkout server-side before fulfilling (the full example lives in [Verify before you fulfill](/sdk/stripe/checkout#verify-before-you-fulfill)):
