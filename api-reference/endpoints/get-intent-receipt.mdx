...
| `failed` | `boolean` | Terminal non-settled state (e.g. the PaymentIntent was canceled) |
| `terminal` | `boolean` | The settlement will not change again. Key terminality on this, never on `status` |
| `externalUrl` | `string` | Stripe-hosted receipt page, once available |

Verify a checkout server-side before fulfilling:
...
async function verifyStripeCheckout(intentId: string, expectedCents: number) {
  ...
  if (!s.settled) throw new Error(`Settlement failed: ${s.status}`);
