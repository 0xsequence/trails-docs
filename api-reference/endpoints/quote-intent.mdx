...
| `checkoutRef` | `string` | Client-generated id for one checkout session. The first quote for a `checkoutRef` creates and arms the payment; re-quotes reuse it. A changed amount, `orderRef` or `chainId` cancels and re-arms under the same ref |
...
Settlement quotes never use the passthrough path, so every settled payment has an intent whose receipt carries the
[settlement outcome](/api-reference/endpoints/get-intent-receipt#settlement). The caller's project must have a Stripe merchant key mapped, or the
quote fails.
