...
- **settlement** (QuoteSettlementParams): Settle the quote in USD on a Stripe account instead of at a destination you specify. See [Stripe settlement quotes](#stripe-settlement-quotes)
...
- **settlement** (QuoteSettlementInfo): Present on Stripe settlement quotes only. The armed Stripe PaymentIntent this quote pays into
...
## Stripe settlement quotes

A settlement quote asks Trails to settle the payment through Stripe instead of delivering tokens to a destination. Stripe pay-with-crypto is the only
settlement provider today (`provider: "stripe"`); it lands the
...
| `amountCents` | `number` | Stripe charge amount in minor units of `currency` |
| `currency` | `string` | Stripe presentment currency, lowercase ISO 4217. Only `"usd"` is accepted today; defaults to `"usd"` |
| `checkoutRef` | `string` | Client-generated id for one checkout session. The first quote for a `checkoutRef` creates and arms the Stripe PaymentIntent; re-quotes reuse it. A changed amount, `orderRef`, `chainId`, or (when `chainId` is omitted) origin chain cancels and re-arms under the same ref |
| `orderRef` | `string` | Merchant order label, recorded in the Stripe PaymentIntent metadata |
| `chainId` | `number` | Chain for the USDC deposit to Stripe. Live Stripe merchant keys: Base `8453`, Polygon `137`, Ethereum `1`. Test keys: Base Sepolia `84532` only. Omitted: the origin chain when it is a Stripe network in the key's mode, else Base |
...
| `currency` | `string` | Stripe presentment currency |
| `amountCents` | `number` | Stripe charge amount as declared at quote time |
| `amountUsd` | `string` | Decimal display amount of the Stripe charge |
| `livemode` | `boolean` | Whether the Stripe merchant key is a live key |
| `chainId` | `number` | Chain the USDC deposit to Stripe lands on |
...
receipt carries the [settlement outcome](/api-reference/endpoints/get-intent-receipt#stripe-settlement). The request must carry a project access key whose
