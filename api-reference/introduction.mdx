...
- Permit signature (if needed) uses infinite approval (maxUint256) for better UX
- Fee collector address comes from `selectedGasFeeOption.feeCollectorAddress`

**Two Scenarios:**
1. **First time / No approval**: Include `permitSignature`, `permitDeadline`, `permitAmount`
2. **Already approved**: Omit permit fields entirely (like the example above - only `intentSignature`, `selectedGasFeeOption`, `userNonce`, `deadline`)
</Note>
