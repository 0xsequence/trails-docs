...
- **[QuoteIntent with `settlement`](/api-reference/endpoints/quote-intent#settlement-quotes)** - Quote a payment that settles in USD on the merchant's Stripe account instead of at a destination address
- **[GetIntentReceipt `settlement`](/api-reference/endpoints/get-intent-receipt#settlement)** - Read the Stripe settlement outcome and hosted receipt to verify a checkout before fulfilling
