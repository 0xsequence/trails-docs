...
This gives developers a straightforward way to build powerful primitives on top of onchain infrastructure: complex multi-protocol flows that would otherwise require multiple transactions, manual coordination, and careful UX handling are reduced to a list of typed steps. Users get a single confirmation with a predictable outcome.

Composable Actions are a typed DSL for describing what happens on the destination chain after Trails bridges and swaps funds. Instead of encoding calldata by hand, you build a list of high-level steps and pass them to `useQuote` or `useTrailsSendTransaction`. Trails handles quoting, bridging, and executing.
