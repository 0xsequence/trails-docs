...
Trails checkout turns "pay with crypto" into a normal Stripe charge. The shopper pays with any token they hold on any supported chain; Stripe pay-with-crypto attributes the resulting USDC deposit to a PaymentIntent on your account, and you settle in USD with a Stripe-hosted receipt.
...
- Add a **Pay with crypto** option next to card and PayPal in an ecommerce checkout
- Sell digital goods, tickets or subscriptions to crypto-holding customers worldwide
- Let marketplaces and platforms accept stablecoins without touching custody
- Cut card fees and chargeback exposure while keeping settlement in USD
...
`onSuccess` waits for Stripe to confirm the deposit, so it fires when the money is actually on your Stripe account, not when the on-chain transfer lands.

### Verify server-side

The price is declared by the browser, so always confirm the settled amount against your order before fulfilling:
...
<CardGroup cols={2}>
  ...
  <Card title="Settlement on the receipt" icon="receipt" href="/api-reference/endpoints/get-intent-receipt#settlement">
