...
| USD on your Stripe account | Any token, any chain | Swap + Bridge + [Stripe checkout](/sdk/stripe/checkout) |
...
<Pay
  ...
  bridgeProvider="CCTP"
  onPaymentSuccess={({ intentId }) => processLargePayment(intentId)}
>
...
export const CrossChainNFTPurchase = () => {
  ...
  return (
    <Pay
      ...
      onPaymentSuccess={({ intentId }) => {
        console.log('NFT purchased:', intentId)
      }}
...
export function CheckoutButton({ order }) {
  ...
  return (
    <Pay
      ...
      onPaymentSuccess={async ({ intentId }) => {
        const verified = await verifyPayment(intentId)
        if (verified) {
          await fulfillOrder(order.id)
          setStatus('complete')
        }
      }}
      onPaymentError={({ sessionId, intentId, error }) => {
        logPaymentError(sessionId, intentId, error)
        setStatus('failed')
      }}
    >
...
export async function verifyPayment(intentId: string, expectedAmount: string) {
...
If you would rather end up with dollars on a Stripe account than tokens at an address, swap the `to` destination for the `stripe` prop on
`TrailsWidget`. The shopper still pays with any token on any chain; Stripe pay-with-crypto attributes the deposit to a PaymentIntent on your
account and issues a hosted receipt.
...
<TrailsWidget
  apiKey="YOUR_API_KEY"
  stripe={{ amountCents: 2500, orderRef: order.id }}
  onSuccess={({ intentId, settlement }) => settlement?.settled && fulfillOrder(order.id, intentId, settlement.reference)}
  renderInline
/>
```

`onSuccess` is held while Stripe confirms settlement and also fires on a failed or timed-out payment, so check `settlement.settled`. See the [Checkout use case](/use-cases/checkout) and the
[Stripe checkout reference](/sdk/stripe/checkout); access is arranged through [Polygon Checkout](https://polygon.technology/checkout).
...
<CardGroup cols={2}>
  ...
  <Card title="Stripe checkout" icon="cart-shopping" href="/use-cases/checkout">
    Settle crypto payments in USD on Stripe
  </Card>
</CardGroup>
