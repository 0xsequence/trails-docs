...
<TrailsWidget
  ...
  onSuccess={({ intentId, settlement }) => fulfillOrder(order.id, intentId, settlement?.reference)}
  renderInline
/>
```

`onSuccess` is held until Stripe confirms settlement. See the [Checkout use case](/use-cases/checkout) and the
