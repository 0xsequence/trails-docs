...
Trails handles all of that in a single transaction. Users can deposit from any token they hold on any supported chain — or use a fiat card onramp ([Stripe](/sdk/stripe/onramp) or Meld), an exchange transfer (Mesh), or a QR code — all in one embedded experience. Fiat purchases land as USDC and route to the destination like any other origin funds, so the destination can be a token or chain the onramp itself does not support.
...
export const BaseDeposit = () => {
  return (
    <Fund
      ...
      onFundingSuccess={({ intentId }) => {
        console.log('Deposit completed:', intentId)
      }}
    />
  )
}
...
export const CustomVaultDeposit = () => {
  ...
  return (
    <Fund
      ...
      onFundingSuccess={({ intentId }) => {
        console.log('Vault deposit completed:', intentId)
      }}
    />
  )
}
...
<Note>
Quotes expire **15 minutes** after `QuoteIntent`. Call `ExecuteIntent` before that window closes or request a new quote. For a Solana or Tron origin, call [`ExecuteIntentEdge`](/api-reference/endpoints/execute-intent-edge) instead — it arms the server-side fill monitor before the user pays on the edge chain.
</Note>
...
- [**Stripe onramp**](/sdk/stripe/onramp) — card funding restrictions, sandbox, and tile placement
