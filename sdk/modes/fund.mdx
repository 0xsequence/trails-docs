...
<Fund
  ...
  onFundingSuccess={({ intentId }) => console.log("funded", intentId)}
/>
```
...
| Prop | Type | Description |
|------|------|-------------|
| `fundMethodsList` | `FundMethodListOption[]` | Ordered list: `"connected-wallet"`, `"stripe-onramp"`, `"cc-onramp"`, `"crypto-transfer"`, `"exchange"` |
...
|----------|-----------|---------------|
| `onFundingStart` | `({ sessionId }) => void` | User begins the funding flow. Fires before quoting, so no `intentId` yet |
| `onFundingSuccess` | `({ sessionId, intentId }) => void` | Funding completes successfully |
| `onFundingError` | `({ sessionId, intentId, error }) => void` | Funding encounters an error |
...
The [Stripe onramp](/sdk/stripe/onramp) is a separate tile from the Meld card flow. It needs no configuration and appears once Stripe is enabled for
your project; pass `onramp.stripe.environment` only for the sandbox, and pin its position with `fundMethodsList`:

```tsx
<Fund
  apiKey="YOUR_API_KEY"
  to={{ recipient: "0x...", currency: "USDC", chain: "base" }}
...
<Fund
  ...
  onFundingSuccess={({ intentId }) => console.log("deposited", intentId)}
/>
```
