...
- Fund mode uses `TradeType.EXACT_INPUT` internally
- User selects input amount; output amount varies based on routes and fees
- `TRAILS_CONTRACT_PLACEHOLDER_AMOUNT` = `uint256.max` internally
- Supports both EOA addresses and smart contract destinations
- Calldata execution is atomic with the transfer
