...
<Earn
  ...
  onEarnSuccess={({ intentId }) => console.log("deposited", intentId)}
/>
```
...
|----------|-----------|---------------|
| `onEarnStart` | `({ sessionId }) => void` | User begins the deposit flow. Fires before quoting, so no `intentId` yet |
| `onEarnSuccess` | `({ sessionId, intentId }) => void` | Deposit completes successfully |
| `onEarnError` | `({ sessionId, intentId, error }) => void` | Deposit encounters an error |
