...
|----------|-----------|---------------|
| `onWithdrawStart` | `({ sessionId }) => void` | User begins the withdrawal flow. Fires before quoting, so no `intentId` yet |
| `onWithdrawSuccess` | `({ sessionId, intentId }) => void` | Withdrawal completes successfully |
| `onWithdrawError` | `({ sessionId, intentId, error }) => void` | Withdrawal encounters an error |
...
<Withdraw
  apiKey="YOUR_API_KEY"
  onWithdrawSuccess={({ intentId }) => console.log("withdrawn", intentId)}
>
