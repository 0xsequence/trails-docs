...
|----------|-----------|---------------|
| `onSwapStart` | `({ sessionId }) => void` | User begins the swap flow. Fires before quoting, so no `intentId` yet |
| `onSwapSuccess` | `({ sessionId, intentId }) => void` | Swap completes successfully |
| `onSwapError` | `({ sessionId, intentId, error }) => void` | Swap encounters an error |
...
<Swap
  apiKey="YOUR_API_KEY"
  onSwapSuccess={({ intentId }) => console.log("swapped", intentId)}
>
...
<Swap
  ...
  onSwapSuccess={({ intentId }) => console.log("bridged", intentId)}
/>
```
