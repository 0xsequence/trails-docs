...
<Pay
  ...
  onPaymentSuccess={({ intentId }) => markOrderPaid(intentId)}
/>
```
...
The payment method screen also shows a **Pay with Stripe** tile whenever the [Stripe onramp](/sdk/stripe/onramp) is enabled for your project. In pay
mode this tile ignores `fundMethodsList` and `hideUnlistedFundMethods`.
...
|----------|-----------|---------------|
| `onPaymentStart` | `({ sessionId }) => void` | User initiates the payment flow. Fires before quoting, so no `intentId` yet |
| `onPaymentSuccess` | `({ sessionId, intentId }) => void` | Payment completes successfully |
| `onPaymentError` | `({ sessionId, intentId, error }) => void` | Payment encounters an error |
...
quotes. See [Callback payloads](/sdk/configuration#callback-payloads).
...
<Pay
  ...
  onPaymentSuccess={({ intentId }) => {
    updateOrderStatus(orderId, "paid", intentId)
  }}
>
...
<Pay
  ...
  onPaymentSuccess={({ intentId }) => console.log("paid", intentId)}
/>
```
...
### Settle in USD with Stripe

To receive dollars on your Stripe account instead of tokens at `to.recipient`, use `TrailsWidget` with the `stripe` prop. It forces pay mode, derives
the destination from the armed Stripe payment, and holds `onSuccess` while Stripe confirms settlement:
...
<TrailsWidget
  apiKey="YOUR_API_KEY"
  stripe={{ amountCents: 5000, orderRef: "order-4821" }}
  onPaymentCreated={({ settlement }) => console.log("stripe payment", settlement.reference)}
  onSuccess={({ intentId, settlement }) => settlement?.settled && markOrderPaid(intentId, settlement)}
  renderInline
/>
```

The focused `<Pay />` component is not used here because its `to` destination conflicts with the server-derived one. Full reference:
[Stripe checkout](/sdk/stripe/checkout).
...
<Pay
  ...
  onPaymentSuccess={({ intentId }) => console.log("minted", intentId)}
>
...
- [Stripe checkout](/sdk/stripe/checkout) — settle payments in USD on Stripe
