...
the destination from the armed Stripe payment, and holds `onSuccess` until Stripe confirms settlement:
...
<TrailsWidget
  ...
  onSuccess={({ intentId, settlement }) => markOrderPaid(intentId, settlement)}
  renderInline
/>
```
