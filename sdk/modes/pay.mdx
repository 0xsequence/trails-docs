...
- Pay mode uses `TradeType.EXACT_OUTPUT` internally
- The widget automatically calculates required input amount including fees
- All bridging and swapping is handled automatically
- Native token payments (ETH, MATIC, etc.) are fully supported
- Calldata execution happens atomically with payment on destination chain
