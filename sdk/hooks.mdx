...
Fetches token balances for a single account with optional pagination.
...
- `options.pageSize`: Number of balances per page (default: 10)
...
type UseTokenBalancesReturn = {
  ...
  page: Page | undefined // Pagination metadata
}
...
Fetches token balances for multiple accounts with optional pagination.
...
**Parameters**:
- `addresses`: Array of wallet addresses to fetch balances for
- `options.page`: Page number for pagination (default: 1)
- `options.pageSize`: Number of balances per page (default: 10)
...
type UseTokenBalancesForMultipleAccountsReturn = {
  balancesByAccount: Record<string, Token[]> // Balances grouped by account address
  isLoading: boolean
  error: Error | null
}
```
