...
Gets the indexer gateway client instance for querying blockchain data.

**Signature**: `() => IndexerGatewayClient`

**Usage**:
```ts
const client = getIndexerGatewayClient()
// Use client to query blockchain data
```

### useIndexerGatewayClient

React hook for accessing the indexer gateway client.

**Signature**: `() => IndexerGatewayClient`

**Usage**:
```tsx
const client = useIndexerGatewayClient()
...
React hook for monitoring meta transaction status.

**Signature**:
```ts
function useMetaTxnsMonitor(params: {
  metaTxnIds?: string[]
  accountAddress?: string
  enabled?: boolean
}): {
...
**Usage**:
```tsx
const { metaTxns, isLoading } = useMetaTxnsMonitor({
  metaTxnIds: ['meta-txn-123'],
  enabled: true,
})
```
...
Gets the receipt for a meta transaction.

**Signature**: `(metaTxnId: string) => Promise<MetaTxnReceipt>`

**Usage**:
```ts
const receipt = await getMetaTxnReceipt('meta-txn-123')
```
...
Main React hook for Trails functionality. Provides access to all core Trails features including account info, chains, tokens, and balances.
...
function useTrails(): {
  ...
  tokenBalances: TokenBalanceExtended[]
...
Gets relayer information for a specific chain.

**Signature**: `(chainId: number) => Promise<Relayer>`

**Usage**:
```ts
const relayer = await getRelayer(1)
console.log(relayer.url)
```

### useRelayers

React hook for accessing relayer information across all chains.

**Signature**:
```ts
function useRelayers(): {
  relayers: Map<number, Relayer>
  isLoading: boolean
}
```
...
**Usage**:
```tsx
const { relayers, isLoading } = useRelayers()
const ethereumRelayer = relayers.get(1)
```

## Logger

```ts
import { logger, getMinLogLevel } from '0xtrails'
```
...
**Usage**:
```ts
logger.info('Swap initiated', { fromToken, toToken })
...
**Signature**: `() => 'debug' | 'info' | 'warn' | 'error'`
...
```ts
import { getTrailsClient, useTrailsClient } from '0xtrails'
```

### getTrailsClient

Gets the Trails client instance for interacting with the Trails protocol.

**Signature**: `() => TrailsClient`
...
React hook for accessing the Trails client.

**Signature**: `() => TrailsClient`

**Usage**:
```tsx
const client = useTrailsClient()
```
