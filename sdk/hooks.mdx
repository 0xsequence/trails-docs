...
<Note>
Added with [Stripe checkout](/sdk/stripe/checkout). Settlement is the post-execution step where an external payment processor (Stripe) confirms the
deposit an intent made. The Trails API owns all processor knowledge; these hooks only read the receipt's `settlement` projection.
</Note>
...
Polls `GetIntentReceipt` for an intent's settlement until it is terminal. Use it on pages that outlive the widget, such as an order confirmation.
...
type IntentSettlementState = {
  ...
  settlement?: IntentSettlement        // undefined when the intent has no processor step
  receiptUrlTimedOut: boolean          // settled, but no receipt URL within the grace window
...
type IntentSettlement = {
  provider: 'stripe'
  label: string          // step label, e.g. "Settle"
  description: string
  ...
  status: string         // raw processor status
  settled: boolean
  failed: boolean
...
- `isSettlementReceiptUnavailable(state)` — `true` when no processor receipt will arrive within the bounds consumers wait on, so a UI can stop showing a spinner.
