...
Pass `settlement` (`QuoteSettlementParams`) to quote a [Stripe checkout](/sdk/stripe/checkout) headlessly. The API derives the destination from the
armed Stripe payment, so leave `to` unset; the returned `quote.settlement` carries the PaymentIntent reference. Settlement quotes never take the
passthrough path.
...
type IntentSettlement = {
  ...
  settled: boolean
  failed: boolean
  terminal: boolean      // key terminality on this, never on status
