...
If your app already uses wagmi, wrap your app with `WagmiProvider` and use wagmi hooks (`useAccount`, `useWalletClient`, etc.) to supply wallet state to Trails hooks. For widget integrations that should share the same wagmi session, pass [`wagmiAdapter`](/sdk/adapters/wagmi) on the widget's `adapters` prop — see [Adapters](/sdk/adapters/overview).
...
For custom EIP-1193 wallets without wagmi, use the `evmAdapter` helper on widget components — see [EIP-1193 adapter](/sdk/adapters/eip-1193).
...
<Note>
Added in [`0xtrails@0.16.0`](/sdk/changelog/0xtrails-0.16.0). Adapters are a **widget-only** feature — pass them on focused widget components (`Pay`, `Fund`, `Swap`, …), not on `TrailsProvider`. See the [Adapters](/sdk/adapters/overview) section for wagmi, EIP-1193, Solana, and thirdweb integrations.
</Note>
...
Pass `settlement` (`QuoteSettlementParams`) to quote a [Stripe checkout](/sdk/stripe/checkout) headlessly. The hook still needs `to.chain`, `to.token`
and `to.recipient` before it quotes: pass the settlement chain and its USDC address as placeholders. The SDK strips them from the request and the API
derives the real destination from the armed Stripe payment; the returned `quote.settlement` carries the PaymentIntent reference. Settlement quotes are
exact-output (no `from.amount`), cannot be combined with `to.calls`, `to.calldata` or `actions`, require an EVM origin, and never take the passthrough
path. See [Headless](/sdk/stripe/checkout#headless) for an example.
...
<Note>
Added in [`0xtrails@0.16.0`](/sdk/changelog/0xtrails-0.16.0). Use these to drive your own fiat on-ramp UI (e.g. payment method picker, exchange selector) outside the Trails widget. Backed by Meld. The [Stripe onramp](/sdk/stripe/onramp) has no hook surface; it is driven entirely by the widget and the Trails API.
</Note>
...
<Warning>
**Deprecated since `0xtrails@0.18.0`.** The SDK send flow no longer calls `CommitIntent` — non-edge intents execute directly from `QUOTED` via `useExecuteIntent`, and origin-mode edge intents arm via [`ExecuteIntentEdge`](/api-reference/endpoints/execute-intent-edge). `useCommitIntent` (and the `commitIntentFn` / `skipCommit` send options) remain exported for existing integrations but are no longer invoked by `send()`.
</Warning>
...
<Note>
Added with [Stripe checkout](/sdk/stripe/checkout). Settlement is the post-execution step where Stripe confirms the USDC deposit an intent made
and the merchant's PaymentIntent succeeds. Stripe is the only settlement provider today. The Trails API owns all Stripe knowledge; these hooks only
read the receipt's `settlement` projection.
</Note>

```ts
import {
  useIntentSettlement,
  isSettlementReceiptUnavailable,
  sanitizeSettlementReceiptUrl,
  resolveIntentSettlementPollInterval,
  getStripeSettlementChainIds,
  INTENT_SETTLEMENT_POLL_MS,
  INTENT_SETTLEMENT_PENDING_HOLD_MS,
  type IntentSettlementState,
  type IntentSettlement,
  type QuoteSettlementParams,
  type QuoteSettlementInfo,
} from '0xtrails'
```

### useIntentSettlement

Polls `GetIntentReceipt` for an intent's Stripe settlement until it is terminal. Use it on pages that outlive the widget, such as an order confirmation.

```tsx
import { getTrailsClient, useIntentSettlement, sanitizeSettlementReceiptUrl } from '0xtrails'

const client = getTrailsClient({ apiKey: 'YOUR_API_KEY' })

function OrderStatus({ intentId }: { intentId: string }) {
  const state = useIntentSettlement({ intentId, client, expectSettlement: true })
  const url = sanitizeSettlementReceiptUrl(state.settlement?.externalUrl)

  if (!state.resolved) return <span>Checking…</span>
  if (state.settlement?.settled) return <a href={url ?? '#'}>Stripe receipt</a>
  if (state.settlement?.failed) return <span>Payment failed</span>
  return <span>Waiting for Stripe…</span>
}
```

| Option | Type | Description |
|--------|------|-------------|
| `intentId` | `string \| null` | Intent to watch |
| `client` | `TrailsClient` | From `getTrailsClient` or `useTrailsClient` |
| `enabled` | `boolean` | Default `true` |
| `track` | `boolean` | `false` reads once instead of polling. Default `true` |
| `expectSettlement` | `boolean` | Set when the quote carried `settlement`, so a receipt without the projection is treated as a transient gap rather than "no settle step" |

#### Types

```ts
type IntentSettlementState = {
  resolved: boolean                    // first read finished
  settlement?: IntentSettlement        // undefined when the intent has no Stripe settle step
  receiptUrlTimedOut: boolean          // settled, but no Stripe receipt URL within the grace window
  awaitingExpectedSettlement: boolean  // expected projection not on the receipt yet
  pendingTimedOut: boolean             // pending past the hold window; consumers should fail open
}

type IntentSettlement = {
  provider: 'stripe'
  label: string          // Stripe settle step label, e.g. "Settle"
  description: string
  reference: string      // Stripe PaymentIntent id
  currency: string       // lowercase ISO 4217, "usd" today
  amountCents: number
  chainId: number        // chain the USDC deposit landed on
  status: string         // raw Stripe PaymentIntent status
  settled: boolean
  failed: boolean
  terminal: boolean      // key terminality on this, never on status; can be true with settled and failed both false (unknown outcome)
  externalUrl?: string   // Stripe-hosted receipt
}
```

### Helpers

- `isSettlementReceiptUnavailable(state)` — `true` when no Stripe receipt will arrive within the bounds consumers wait on, so a UI can stop showing a spinner.
- `sanitizeSettlementReceiptUrl(url)` — returns the URL only for `http(s)` schemes; use it before rendering `externalUrl` in an `<a href>`.
- `resolveIntentSettlementPollInterval(settlement, settledForMs, pendingForMs)` — the poll cadence the hook uses, exported for custom pollers.
- `getStripeSettlementChainIds(mode)` — chain ids a Stripe merchant key can settle on: `[8453, 137, 1]` for `'live'`, `[84532]` for `'test'`.
...
<Warning>
**Deprecated since `0xtrails@0.18.0`.** The SDK's `send()` no longer invokes `commitIntent`. New integrations should call [`executeIntent`](/api-reference/endpoints/execute-intent) directly on the `QUOTED` intent returned by [`useQuote`](#usequote), or [`executeIntentEdge`](/api-reference/endpoints/execute-intent-edge) for origin-mode edge intents. The export remains for existing integrations but is no longer invoked internally.
</Warning>
