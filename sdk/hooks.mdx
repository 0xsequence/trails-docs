...
type UseQuoteProps = {
  ...
  from: {
    ...
    amount?: string | bigint // EXACT_INPUT when set
  }
  to: {
    token: string
    chain: ChainIdentifier
    ...
    approve?: string | null
    amount?: string | bigint // EXACT_OUTPUT when set
...
```ts
import { useResolveEns, useResolveEnsAddress, useResolveEnsName } from '0xtrails'
```

- `useResolveEns({ address })`: Returns `{ ensName, ensAvatar, isLoading }`
- `useResolveEnsAddress({ textInput })`: Resolves `.eth` names to addresses
- `useResolveEnsName({ address })`: Resolves an address to its ENS name

ENS lookups are cached for 24 hours to minimize repeated resolution calls.

## Intent Storage & Monitoring
...
- `storeIntentForMonitoring(intentId, data)`: Persist intent metadata for background tracking
- `getStoredIntents()`: Read stored intents for monitoring
- `removeStoredIntent(intentId)`: Remove a stored intent
- `clearAllStoredIntents()`: Clear all stored intents
