...
| `onPaymentCreated` | `{ sessionId, settlement }` | A quote armed an external payment (Stripe checkout). Fires once per payment reference, at quote time |
...
`IntentSettlement` describing the confirmed Stripe payment, and the widget holds that callback until Stripe confirms the deposit.
...
| Prop | Type | Description |
|------|------|-------------|
| `stripe` | `TrailsStripeCheckoutProps` | `{ amountCents, orderRef?, chainId?, testMode? }`. Prices the checkout in USD cents and settles it on your Stripe account. Forces pay mode; mutually exclusive with `toAddress`, `toAmount`, `toChainId`, `toToken`, `toCalldata`, `actions` |
...
| `onPaymentCreated` | `({ sessionId, settlement }) => void` | Armed payment reference, amount and chain, available at quote time |
