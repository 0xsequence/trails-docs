...
For [Stripe checkout](/sdk/stripe/checkout) integrations the success payload (`onSuccess` on `TrailsWidget`) also carries `settlement`, an
`IntentSettlement` describing the Stripe payment. The widget holds that callback while settlement is pending and releases it when Stripe confirms
the deposit, the payment fails, or the pending hold times out, so check `settlement.settled` before treating the order as paid.

### Stripe checkout props

These props live on `TrailsWidget` (imported from `0xtrails`) rather than the focused components, because they replace the destination.

| Prop | Type | Description |
|------|------|-------------|
| `stripe` | `TrailsStripeCheckoutProps` | `{ amountCents, orderRef?, chainId?, testMode? }`. Prices the checkout in USD cents and settles it on your Stripe account. Forces pay mode; mutually exclusive with `toAddress`, `toAmount`, `toChainId`, `toToken`, `toCalldata`, `actions` and `settlement` |
| `merchantCheckout` | `boolean` | Merchant-checkout presentation: one processing state while pending, receipt keeps only payment links. Implied by `stripe`; set explicitly to opt any integration in or out |
| `settlement` | `QuoteSettlementParams` | Advanced: raw settlement quote params, when you manage the checkout reference yourself. Set automatically by `stripe` |
| `onPaymentCreated` | `({ sessionId, settlement }) => void` | A quote armed an external payment. Fires once per payment reference, at quote time, with the reference, amount and chain. `TrailsWidget` only; the focused components do not expose it |

See [Stripe checkout](/sdk/stripe/checkout) for the flow, verification guidance and headless equivalents.
...
The [Stripe onramp](/sdk/stripe/onramp) is its own tile rather than a `paymentMethod` value. It appears whenever Stripe is enabled for your project;
order or hide it with `fundMethodsList` (`"stripe-onramp"`), or open it directly on `TrailsWidget` with `fundMethod="onramp-stripe"`.
