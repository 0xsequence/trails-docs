...
description: "Accept any token on any chain and settle in USD on your Stripe account with the widget stripe prop"
sidebarTitle: "Checkout"
---

Stripe checkout is Trails' merchant settlement path. You state a price in USD; the shopper pays with whatever token they hold on any supported chain; Trails routes it into a USDC deposit that Stripe pay-with-crypto attributes to a PaymentIntent on **your** Stripe account. You settle in USD like any other Stripe charge, with a Stripe-hosted receipt.

All Stripe knowledge lives in the Trails API. The SDK forwards a settlement request through the normal quote flow and tracks the settle step on the intent receipt. No Stripe key ever reaches the browser, and no merchant backend is required to take a payment.

Requires `0xtrails@0.19.0` or later. The wire types shipped earlier in `@0xtrails/api@0.18.5`.

<Note>
Stripe checkout is offered through [Polygon Checkout](https://polygon.technology/checkout). Book a demo or talk to sales there to get a Stripe merchant key mapped to your Trails project; the `stripe` prop fails at quote time until one is configured.
</Note>
...
<TrailsWidget
  ...
  onSuccess={({ intentId, settlement }) => {
    // Fires once Stripe confirms the deposit (or once the pending hold fails open)
...
That is the whole integration. The `stripe` prop forces pay mode and derives the destination server-side, so the destination props (`toAddress`, `toAmount`, `toChainId`, `toToken`, `toCalldata`, `actions`) must be left unset. Passing any of them alongside `stripe` throws at render.

<Note>
Use `TrailsWidget` from `0xtrails` for Stripe checkout. The focused `<Pay />` component requires a `to` destination, which conflicts with the server-derived one.
</Note>
...
| `onSuccess` | `CheckoutEventBase & { settlement?: IntentSettlement }` | Held until Stripe confirms the deposit. Carries `intentId` for receipt lookups and the confirmed `settlement`. |
...
`IntentSettlement` adds the live state: `status` (raw Stripe status), `settled`, `failed`, `terminal`, and `externalUrl` (the Stripe-hosted receipt once available). Key terminality on `terminal`, never on the status string.
...
export async function verifyStripeCheckout(intentId: string, order: { totalCents: number; ref: string }) {
  ...
  if (!s.settled) return { ok: false, reason: s.failed ? 'payment failed' : 'not settled yet' }
...
An order confirmation page that outlives the widget can poll the receipt itself:

```tsx
import { getTrailsClient, useIntentSettlement, sanitizeSettlementReceiptUrl } from '0xtrails'

const client = getTrailsClient({ apiKey: 'YOUR_TRAILS_API_KEY' })

export function OrderConfirmation({ intentId }: { intentId: string }) {
  const { settlement, resolved } = useIntentSettlement({ intentId, client, expectSettlement: true })
  const receiptUrl = sanitizeSettlementReceiptUrl(settlement?.externalUrl)

  if (!resolved) return <p>Checking payment…</p>
  if (settlement?.settled) return <a href={receiptUrl ?? '#'}>View Stripe receipt</a>
  if (settlement?.failed) return <p>Payment was not completed.</p>
  return <p>Waiting for Stripe to confirm…</p>
}
```

See [Settlement](/sdk/hooks#settlement) in the hooks reference for the full state shape.
...
- `useQuote` accepts a `settlement` option (`QuoteSettlementParams`: `provider: 'stripe'`, `amountCents`, `checkoutRef`, `orderRef?`, `chainId?`) and the quote result carries `settlement`. Leave every destination field unset when you pass it.
...
The API surface is documented under [`QuoteIntent`](/api-reference/endpoints/quote-intent#settlement-quotes) and [`GetIntentReceipt`](/api-reference/endpoints/get-intent-receipt#settlement).
...
| | Stripe checkout | [Stripe onramp](/sdk/stripe/onramp) |
|---|---|---|
| Direction | Crypto in, USD out (to the merchant) | Card in, USDC out (to the intent) |
| Configured by | `stripe` prop | Nothing; `onramp.stripe.environment` for sandbox |
| Requires | Stripe merchant key on the project | Stripe enabled for the project |
| Modes | Pay (forced) | Fund, swap, pay |

The two can be combined: a checkout with the `stripe` prop can still offer the Stripe onramp tile as one way for the shopper to fund the payment.
