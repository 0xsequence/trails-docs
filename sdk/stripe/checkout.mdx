...
An order confirmation page that outlives the widget can poll the receipt itself with `useIntentSettlement`. See [Settlement](/sdk/hooks#settlement) in the hooks reference for the hook, its state shape and a full example.
