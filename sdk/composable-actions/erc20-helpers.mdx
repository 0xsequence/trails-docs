...
Side-effect-free helpers for building ERC-20 calls and looking up tokens. Pair them with [`actionBuilder.custom`](/sdk/composable-actions/building-actions#custom-arbitrary-contract-call-escape-hatch) for ergonomic custom calldata.
...
Each returns a `Call` — drop it directly into `actionBuilder.custom(...)`:
