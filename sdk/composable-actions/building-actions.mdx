...
description: "Full actionBuilder API: swap, lend, deposit, assert, and build"
sidebarTitle: "Building Actions"
---

```ts
import { actionBuilder, dynamic, self } from '0xtrails'

const { swap, lend, deposit, assert, build } = actionBuilder
```

Amounts are always one of:

- **`string`** — human-readable, e.g. `"50"` or `"0.03"`. Scaled by the token's decimals.
- **`bigint`** — raw wei, used as-is.
- **`dynamic()`** — "use the intent wallet's runtime balance at execution time". See [Dynamic Values](/sdk/composable-actions/dynamic-values).
...
- **Custom compositions** — build `Call[]` directly and embed them with [`build(...)`](#build-arbitrary-contract-call-escape-hatch) when you need something the typed `swap` doesn't cover (e.g. multi-hop paths, permit2, …).
...
Build an approval + swap `Call[]` for an exact-input swap. Wrap in [`build(...)`](#build-arbitrary-contract-call-escape-hatch) to embed inside an intent, or send directly from a wallet.
...
Use for Aave / Compound / Fluid-style lending. **Use [`useEarnMarkets`](/sdk/composable-actions/markets-and-providers) to fetch the `marketId`.**
...
<Info>
`assert` is the only builder that does **not** accept `dynamic()` — its values must be known at quote time.
</Info>

## `build` — arbitrary contract call (escape hatch)
