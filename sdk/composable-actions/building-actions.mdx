...
description: "Full actionBuilder API: swap, lend, deposit, assert, and custom"
...
- **Custom compositions** — build `Call[]` directly and embed them with [`custom(...)`](#custom-arbitrary-contract-call-escape-hatch) when you need something the typed `swap` doesn't cover (e.g. multi-hop paths, permit2, …).
...
Build an approval + swap `Call[]` for an exact-input swap. Wrap in [`custom(...)`](#custom-arbitrary-contract-call-escape-hatch) to embed inside an intent, or send directly from a wallet.
