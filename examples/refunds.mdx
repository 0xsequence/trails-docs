...
- **User "sudo" Control**: Users can take direct action over their funds at any time, even in emergencies
...
While Trails automatically refunds upon transaction failures, there may be edge cases where funds are stuck without an explicit revert. The `useTrailsRefund` hook provides utilities to manually sign and execute refund transactions to recover funds from intent addresses.
