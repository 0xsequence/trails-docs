...
Recovery is implemented in the [Trails Explorer](https://app.trails.build/explorer) as well as in the Transaction History within the Trails Widget which will automatically detect intents that are eligible for a recovery.
...
While Trails automatically refunds upon transaction failures, there may be edge cases where funds are stuck without an explicit revert. 
 ...
 The `useTrailsRefund` hook provides utilities to manually sign and execute refund transactions to recover funds from intent addresses if you'd like to add to your own app.
...
- [**Hooks Reference**](/sdk/hooks#usetrailsrefund) — Full documentation for `useTrailsRefund`
